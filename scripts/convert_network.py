...
def generate_python_module(networks, output_path, source_url):
    if not networks:
        print("No valid network data found, cannot generate Python module.", file=sys.stderr)
        return

    python_networks = {}
    for n, data in sorted(networks.items()):
        pairs = [pair for stage in data['stages'] for pair in stage]
        python_networks[n] = pairs
    ...
    for n, pairs in sorted(python_networks.items()):
        output_lines.append(f"    {n}: {pairs},")
    ...
    try:
        ...
        print(f"Successfully generated Python module: {output_path}", file=sys.stderr)
...
def main():
    ...
    python_output_path = os.path.join("..", "exprutils", "_sorting_networks.py")
    ...
    generate_python_module(networks_data, python_output_path, source_url)
